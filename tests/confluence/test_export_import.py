...
def test_strip_sensitive_data_dict():
    out = export_import._strip_sensitive_data({"file_contents": "x", "template_id": "t"})
...
def test_load_schema_with_file():
    with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as f:
        json.dump({"required": ["version", "examples"], "definitions": {"example": {"required": ["content_profile"]}}}, f)
        path = f.name
    try:
...
def test_validate_export_payload_missing_required():
    errs = export_import._validate_export_payload({"examples": []}, {"required": ["version", "exported_at", "examples"]})
    assert any("version" in e or "exported_at" in e for e in errs)


def test_validate_export_payload_examples_not_list():
    errs = export_import._validate_export_payload({"version": 1, "exported_at": "x", "examples": "not-list"}, {})
...
def test_validate_export_payload_example_not_dict():
    errs = export_import._validate_export_payload(
        ...
        {"definitions": {"example": {"required": ["content_profile", "template_ref", "intelligence_metrics"]}}},
    )
...
def test_validate_export_payload_template_ref_invalid():
    errs = export_import._validate_export_payload(
        {"version": 1, "exported_at": "x", "examples": [{"content_profile": {}, "template_ref": {}, "intelligence_metrics": {}}]},
        {"definitions": {"example": {"required": ["content_profile", "template_ref", "intelligence_metrics"]}}},
    )
...
def test_validate_import_payload_valid():
    payload = {"version": 1, "exported_at": "2020-01-01T00:00:00", "examples": [
        {"content_profile": {}, "template_ref": {"template_id": "t", "template_name": "T"}, "intelligence_metrics": {}}
    ]}
...
def test_import_examples_empty_list():
    n, msg = export_import.import_examples({"version": 1, "exported_at": "2020-01-01T00:00:00", "examples": []})
    assert n == 0
    assert "0 new" in msg
...
def test_import_examples_valid_file_based():
    with tempfile.TemporaryDirectory() as tmp:
        payload = {
            ...
            "examples": [
                {"content_profile": {"lang": "py"}, "template_ref": {"template_id": "t1", "template_name": "T1"}, "intelligence_metrics": {}}
            ],
        }
...
def test_import_examples_merge_skips_duplicate():
    with tempfile.TemporaryDirectory() as tmp:
        payload = {
            ...
            "exported_at": "2020-01-01T00:00:00",
            "examples": [
                {"content_profile": {"x": 1}, "template_ref": {"template_id": "t", "template_name": "T"}, "intelligence_metrics": {}},
                {"content_profile": {"x": 1}, "template_ref": {"template_id": "t", "template_name": "T"}, "intelligence_metrics": {}},
            ],
        }
