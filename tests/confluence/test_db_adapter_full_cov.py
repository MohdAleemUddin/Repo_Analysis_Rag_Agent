...
def test_get_connection_with_url():
    with patch.object(db_adapter, "get_confluence_config", return_value={"database_url": "postgres://x"}):
...
def test_db_fetch_examples_with_conn_and_filters():
    ...
    mock_conn.cursor.return_value = mock_cur
    with patch.object(db_adapter, "get_connection", return_value=mock_conn):
        out = db_adapter.db_fetch_examples(from_date="2024-01-01", to_date="2024-12-31", template_id="t1")
    assert len(out) == 1
...
def test_db_update_template_confidence_success():
    ...
    mock_conn.cursor.return_value = mock_cur
    with patch.object(db_adapter, "get_connection", return_value=mock_conn):
        ok = db_adapter.db_update_template_confidence("00000000-0000-0000-0000-000000000001", 0.95)
    assert ok is True
