...
_edge = _root / "offline-folder-rag" / "edge_agent"
...
def test_mcp_task_is_rag_is_confluence():
    from app.mcp_server.task_queue import MCPTask, RAG_QUERY, CONFLUENCE_INTELLIGENT_ANALYZE
...
def test_enqueue_dequeue_rag_first_then_confluence():
    ...
    t2 = dequeue()
    assert t2 is not None and t2.task_type == CONFLUENCE_INTELLIGENT_ANALYZE and t2.task_id == "c1"
    assert is_empty() is True
    assert dequeue() is None
...
def test_enqueue_confluence_high_priority_before_normal():
    ...
    enqueue(MCPTask(task_id="n1", task_type=CONFLUENCE_INTELLIGENT_ANALYZE, high_priority=False))
    enqueue(MCPTask(task_id="h1", task_type=CONFLUENCE_INTELLIGENT_ANALYZE, high_priority=True))
    t1 = dequeue()
...
def test_run_confluence_task_match_format_create():
    ...
    for task_type in (CONFLUENCE_INTELLIGENT_MATCH, CONFLUENCE_INTELLIGENT_FORMAT, CONFLUENCE_INTELLIGENT_CREATE):
        task = MCPTask(task_id="x", task_type=task_type, params={})
        out = run_confluence_task(task)
        assert out["status"] == "success"
    task_create = MCPTask(task_id="c1", task_type=CONFLUENCE_INTELLIGENT_CREATE, params={})
...
def test_run_confluence_task_with_progress_and_cancel():
    ...
    task = MCPTask(task_id="a1", task_type=CONFLUENCE_INTELLIGENT_ANALYZE, params={})
    out = run_confluence_task(task, progress_callback=progress_cb, cancel_check=lambda: False)
    assert out["status"] == "success"
    assert len(progress_log) >= 1
    out_cancel = run_confluence_task(task, progress_callback=None, cancel_check=lambda: True)
...
def test_run_confluence_task_cancel_in_match_format_create():
    ...
    from app.mcp_server.task_queue import MCPTask, CONFLUENCE_INTELLIGENT_MATCH, CONFLUENCE_INTELLIGENT_FORMAT, CONFLUENCE_INTELLIGENT_CREATE
    for fn, tt in [
...
def test_confluence_intelligent_entry_points():
    ...
    from app.mcp_server.task_queue import MCPTask, CONFLUENCE_INTELLIGENT_ANALYZE, CONFLUENCE_INTELLIGENT_MATCH, CONFLUENCE_INTELLIGENT_FORMAT, CONFLUENCE_INTELLIGENT_CREATE
    for fn, tt in [
...
def test_orchestrator_lifecycle_success():
    from app.mcp_server.task_queue import MCPTask, CONFLUENCE_INTELLIGENT_ANALYZE, enqueue, is_empty
    from app.mcp_server.orchestrator import orchestrate, set_progress_callback, set_cancel_check, LIFECYCLE_PROCESSING, LIFECYCLE_SUCCESS
    progress_log = []
    set_progress_callback(lambda tid, status, value: progress_log.append((tid, status, value)))
...
def test_orchestrator_lifecycle_learning_trigger():
    from app.mcp_server.task_queue import MCPTask, CONFLUENCE_INTELLIGENT_CREATE, enqueue
    from app.mcp_server.orchestrator import orchestrate, set_progress_callback, set_cancel_check, LIFECYCLE_LEARNING
    progress_log = []
    set_progress_callback(lambda tid, status, value: progress_log.append((tid, status, value)))
    set_cancel_check(lambda _: False)
    enqueue(MCPTask(task_id="learn1", task_type=CONFLUENCE_INTELLIGENT_CREATE, params={}))
    orchestrate()
    statuses = [p[1] for p in progress_log]
...
def test_orchestrator_cancelled_task():
    from app.mcp_server.task_queue import MCPTask, CONFLUENCE_INTELLIGENT_ANALYZE, enqueue, is_empty
    from app.mcp_server.orchestrator import orchestrate, set_progress_callback, set_cancel_check, LIFECYCLE_FAILURE
    progress_log = []
    set_progress_callback(lambda tid, status, value: progress_log.append((tid, status, value)))
    set_cancel_check(lambda tid: tid == "cancel1")
    enqueue(MCPTask(task_id="cancel1", task_type=CONFLUENCE_INTELLIGENT_ANALYZE, params={}))
    orchestrate()
    statuses = [p[1] for p in progress_log]
...
def test_orchestrator_rag_task_stub():
    ...
    from app.mcp_server.orchestrator import orchestrate, set_progress_callback, set_cancel_check, LIFECYCLE_SUCCESS
    progress_log = []
    set_progress_callback(lambda tid, status, value: progress_log.append((tid, status, value)))
    set_cancel_check(lambda _: False)
...
def test_orchestrator_transient_retry_then_fail():
    from app.mcp_server.confluence_tasks import run_confluence_task
    from app.mcp_server.task_queue import MCPTask, CONFLUENCE_INTELLIGENT_ANALYZE, enqueue, max_retries
    from app.mcp_server.orchestrator import orchestrate, set_progress_callback, set_cancel_check
    progress_log = []
    set_progress_callback(lambda tid, status, value: progress_log.append((tid, status, value)))
    set_cancel_check(lambda _: False)
    call_count = [0]
    ...
    with patch("app.mcp_server.orchestrator.confluence_tasks.run_confluence_task", side_effect=failing_task):
        enqueue(MCPTask(task_id="retry1", task_type=CONFLUENCE_INTELLIGENT_ANALYZE, params={}))
...
def test_orchestrator_non_transient_fail_fast():
    ...
    with patch("app.mcp_server.orchestrator.confluence_tasks.run_confluence_task", side_effect=auth_fail):
        enqueue(MCPTask(task_id="auth1", task_type=CONFLUENCE_INTELLIGENT_ANALYZE, params={}))
        orchestrate()
    statuses = [p[1] for p in progress_log]
    assert LIFECYCLE_FAILURE in statuses
    assert is_empty() is True
...
def test_orchestrator_rate_limit_backoff():
    from app.mcp_server.task_queue import MCPTask, CONFLUENCE_INTELLIGENT_ANALYZE, enqueue
    from app.mcp_server.orchestrator import orchestrate, set_progress_callback, set_cancel_check
    ...
    def rate_limit_then_ok(*args, **kwargs):
        ...
        return {"status": "success", "result": {}}
    with patch("app.mcp_server.orchestrator.confluence_tasks.run_confluence_task", side_effect=rate_limit_then_ok):
        with patch("app.mcp_server.orchestrator._exponential_backoff") as mock_backoff:
            enqueue(MCPTask(task_id="rl1", task_type=CONFLUENCE_INTELLIGENT_ANALYZE, params={}))
            orchestrate()
