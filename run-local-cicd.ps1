...
# Repo-specific paths (only include paths that exist)
$PythonPaths = @()
foreach ($path in @("repo_analysis_rag/", "offline-folder-rag/edge_agent/", "tests/")) {
    if (Test-Path $path) {
        $PythonPaths += $path
    }
}
...
'@
    ...
    if ($runPython) {
        if ($runUnit) {
            ...
            $testDirs = @()
            if (Test-Path "$repoRoot\tests\confluence") { $testDirs += "$repoRoot\tests\confluence" }
            if (Test-Path "$repoRoot\tests\edge_agent\unit") { $testDirs += "$repoRoot\tests\edge_agent\unit" }
