...
def _format_single_type(content: str) -> str:
    ...
    is_code_like = any(s in strip for s in code_signals) or (
        strip.count("\n") > 5 and "  " in strip
    )
    if is_code_like:
...
def _format_mixed_content(content: str) -> str:
    ...
    overview_html = (
        "\n".join(
            f"<p>{line}</p>" if line.strip() else "<p></p>" for line in overview_lines
        )
        if overview_lines
        else "<p>Content overview.</p>"
    )
    rest_html = (
        "\n".join(
            f"<p>{line}</p>" if line.strip() else "<p></p>" for line in rest_lines
        )
        if rest_lines
        else ""
    )
