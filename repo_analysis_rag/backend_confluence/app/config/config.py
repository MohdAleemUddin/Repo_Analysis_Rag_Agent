...
def _merge_confluence_settings_from_env(out: dict[str, Any]) -> None:
    ...
    if retry is not None:
        try:
            ...
            pass
    if os.environ.get("CONFLUENCE_LEARNING_ENABLED", "").strip().lower() in (
        "0",
        "false",
        "no",
    ):
...
def get_confluence_config(workspace_id: str | None = None) -> dict[str, Any]:
    ...
    team_sharing_opt_in = (
        opt_in not in ("0", "false", "no") if opt_in else _DEFAULT_TEAM_SHARING_OPT_IN
    )
    out: dict[str, Any] = {
    ...
    if workspace_id is not None:
        creds = get_workspace_credentials(
            service="confluence", workspace_id=workspace_id
        )
