...
def test_connection(url: str, email: str, api_token: str) -> dict[str, Any]:
    ...
    if data is None:
        if status_code == 401:
            ...
            result["error"] = (
                "Confluence returned 403. Check permissions or space access."
            )
        elif status_code == 404:
            result["error"] = (
                "Confluence returned 404. Check Confluence URL (include /wiki)."
            )
        elif status_code is not None:
            result["error"] = f"Confluence returned {status_code}."
        elif parse_error:
            result["error"] = (
                "Confluence returned 200 but response was not JSON (login page or block?)."
            )
        else:
            result["error"] = (
                "Connection failed (timeout or network). Check URL and connectivity."
            )
        return result
    result["ok"] = True
    result["error"] = ""
