...
def _validate_export_payload(data: dict[str, Any], schema: dict[str, Any]) -> list[str]:
    ...
    if "examples" in data:
        if not isinstance(data["examples"], list):
            ...
            req = defn.get(
                "required", ["content_profile", "template_ref", "intelligence_metrics"]
            )
            for i, ex in enumerate(data["examples"]):
                ...
                if "template_ref" in ex and isinstance(ex["template_ref"], dict):
                    if (
                        "template_id" not in ex["template_ref"]
                        or "template_name" not in ex["template_ref"]
                    ):
                        errs.append(
                            f"examples[{i}].template_ref must have template_id and template_name."
                        )
    return errs
...
def export_examples(
    ...
    for ex in examples:
        ...
        if (
            ...
            and "confidence_score" not in metrics
            and ex.get("confidence_score") is not None
        ):
        ...
        out.append(
            {
                ...
                "user_feedback": (
                    ex.get("user_feedback")
                    if isinstance(ex.get("user_feedback"), (int, type(None)))
                    else None
                ),
    ...
    if len(examples) >= 100 and elapsed > 3.0:
        ...
        logging.warning(
            "NFR1: export took %.2fs for %d examples (target <3s)",
            elapsed,
            len(examples),
        )
...
def import_examples(
    ...
    for ex in examples_list:
        ...
        metrics = ex.get("intelligence_metrics") or {}
        profile_hash = ex.get("content_profile_hash") or _content_profile_hash(
            content_profile
        )
        ...
        eid, _ = store_example(
            ...
            confidence_score=(
                metrics.get("confidence_score") if isinstance(metrics, dict) else None
            ),
    ...
    if len(examples_list) >= 100 and elapsed > 5.0:
        ...
        logging.warning(
            "NFR1: import took %.2fs for %d examples (target <5s)",
            elapsed,
            len(examples_list),
        )
