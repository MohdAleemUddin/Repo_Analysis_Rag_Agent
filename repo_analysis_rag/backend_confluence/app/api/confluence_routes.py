...
def _get_json_body(request: Any) -> Any:
    ...
    if hasattr(request, "body"):
        try:
            ...
            return _json.loads(
                getattr(request.body, "decode", lambda x: x)(
                    request.body
                    if callable(getattr(request.body, "decode", None))
                    else request.body
                )
            )
        except Exception:
...
def track_usage_handler(body: dict[str, Any]) -> dict[str, Any]:
    ...
    _confluence_usage_store[key].append(
        {"feature": feature, "timestamp": __import__("time").time()}
    )
...
def intelligent_analyze_handler(body: dict[str, Any]) -> dict[str, Any]:
    ...
    if chat_context and isinstance(chat_context, dict):
        messages = (
            chat_context.get("messages") or chat_context.get("last_messages") or []
        )
        selected_text = (
            chat_context.get("selected_text") or chat_context.get("selection") or ""
        )
        workspace_path = (
            chat_context.get("workspace_path")
            or chat_context.get("workspacePath")
            or ""
        )
        try:
        ...
        if context_result:
            return {
                "context_suggestions": {
                    ...
                    "project_type_label": context_result.get(
                        "project_type_label", "Mixed project"
                    ),
                    "detected_language": context_result.get(
                        "detected_language", "Mixed"
                    ),
                    "should_suggest_readme": context_result.get(
                        "should_suggest_readme", False
                    ),
                },
            }
...
def intelligent_create_handler(body: dict[str, Any]) -> dict[str, Any]:
    ...
    if auto_title:
        ...
        title = intelligence_context.get("title_override") or body.get(
            "title", "Untitled"
        )
    ...
    tid = (
        intelligence_context.get("template_id") or rec.get("template_id") or ""
    ).strip()
    if tid:
        tname = (
            intelligence_context.get("template_name") or rec.get("template_name") or tid
        ) or ""
    ...
    if isinstance(files, list) and files and not isinstance(files[0], str):
        file_contents = [
            c.get("content", "") if isinstance(c, dict) else str(c) for c in files
        ]
    else:
...
def _error_response(
    ...
    return {
        ...
        or "Check request format and try again.",
...
def intelligence_status_handler(
    request: Any = None,
    ...
    if request is not None:
        ...
        team_count = get_collective_intelligence_count(
            ...
            team_sharing_opt_in=get_confluence_config().get(
                "team_sharing_opt_in", True
            ),
        )
...
def intelligence_feedback_handler(
    body: dict[str, Any], request: Any = None
    ...
    if creation_id is not None and intelligence_score is not None:
        ...
        if score is not None and 1 <= score <= 5:
            ...
            data = get_intelligence_status(
                detail_level="full", db_fetch_metrics=db_fetch_intelligence_metrics
            )
            return (
                {
                    "updated": True,
...
def register_confluence_routes(router: Any) -> None:
    ...
    if hasattr(router, "post") and hasattr(router, "get"):
        if Body is not None:
            ...
            router.post("/confluence/intelligent-analyz")(
                analyze_route
            )  # alias for client typo
            router.get("/confluence/intelligent-analyz")(
                status_route
            )  # GET typo -> status
            ...
            router.post("/confluence/intelligence-feedback")(feedback_route)
            router.post("/confluence/track-usage")(track_usage_route)
            router.route("/confluence/examples/export", methods=["GET"])(
                examples_export_handler
            )
            router.route("/confluence/examples/import", methods=["POST"])(
