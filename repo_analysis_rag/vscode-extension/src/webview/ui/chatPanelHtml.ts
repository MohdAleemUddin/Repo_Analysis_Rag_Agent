...
export function getChatPanelHtml(extensionUri: vscode.Uri, placeholderText: string = PLACEHOLDER_TEXT, panelNonce?: string): string {
    const nonce = panelNonce ?? getNonce();
    const firstTimeTooltipJs = escapeForJs(messages.phrases.firstInteraction.buttonTooltip);
    const saveToConfluenceLabelJs = escapeForJs(messages.buttons.saveToConfluence);
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8" />
