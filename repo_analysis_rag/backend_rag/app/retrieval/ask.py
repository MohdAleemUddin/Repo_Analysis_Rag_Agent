...
def _map_to_citation(
    metadata: Mapping[str, Any] | None, document: str | None
) -> dict[str, Any]:
...
def _dedupe_key(citation: Mapping[str, Any]) -> tuple[str, int, int]:
    ...
    if raw_line_end is None:
        raw_line_end = (
            citation.get("line_start") or citation.get("line_number") or line_start
        )
...
def retrieve_vector_results(
    ...
    code_hits = _query_collection(
        client, code_collection, normalized_query, effective_top_k
    )
    doc_hits = _query_collection(
        client, doc_collection, normalized_query, effective_top_k
    )

    rg_hits: list[dict[str, Any]] = []
    if search_root:
        try:
            raw_matches = execute_ask_query(
                search_root, normalized_query, limit=ripgrep_limit
            )
