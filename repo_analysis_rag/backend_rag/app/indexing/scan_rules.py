...
def normalize_root_path(raw_path: str) -> str:
    ...
    if norm_path.endswith("\\") and not (
        len(norm_path) == 3 and norm_path[1:3] == ":\\"
    ):
...
def scan_repository(
    root_path: str, mode: str = "full", changed_files: list[str] | None = None
) -> dict:
    ...
    try:
        ...
        if os.name == "nt":
            ...
            try:
                ...
                return bool(
                    lstat_res.st_file_attributes & stat.FILE_ATTRIBUTE_REPARSE_POINT
                )
