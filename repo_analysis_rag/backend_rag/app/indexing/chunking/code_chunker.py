...
def _chunk_metadata(
    path: Path | str, chunk_id: int, line_start: int, line_end: int
) -> Dict[str, Any]:
    return {
        "path": str(path),
...
def _find_boundaries(
    text: str, lines: List[str], hint: str | None
) -> List[Tuple[int, int, str]]:
    candidates: List[Tuple[int, int, str]] = []
    seen_lines: set[int] = set()
    pattern_sources = (
        [(hint, BOUNDARY_PATTERNS[hint])]
        if hint and hint in BOUNDARY_PATTERNS
        else BOUNDARY_PATTERNS.items()
    )
...
def chunk(file_path: Path | str, *args, **kwargs) -> Iterable[Dict[str, Any]]:
    ...
    while idx < len(boundaries):
        ...
        next_boundary_line = (
            boundaries[idx + 1][0] if idx + 1 < len(boundaries) else total_lines + 1
        )
        natural_end_line = min(next_boundary_line - 1, total_lines)
        if natural_end_line < boundary_line:
            natural_end_line = total_lines
        while (
            natural_end_line > boundary_line
            and natural_end_line <= total_lines
            and not lines_all[natural_end_line - 1].strip()
        ):
            natural_end_line -= 1
        ...
        if kind == "python":
            ...
            base_indent = len(lines_all[base_idx]) - len(
                lines_all[base_idx].lstrip(" ")
            )
            ...
            if end_line == max_line and max_line < natural_end_line:
                ...
                if remainder_line <= total_lines and remainder_line not in seen_lines:
                    pos = (
                        line_offsets[remainder_line - 1]
                        if remainder_line - 1 < len(line_offsets)
                        else line_offsets[-1]
                    )
