...
export type ConfluenceMessageType =
  | 'file_selector'
  | 'analysis'
  | 'progress'
  | 'success'
...
export interface ConfluenceMessage {
  id: string;
  type: ConfluenceMessageType;
  html: string;
}
...
function getChatViewHtml(webview: vscode.Webview): string {
...
export function getFileSelectorMessageHtml(): string {
...
export function getProgressMessageHtml(steps: Array<{ label: string; status: string }>, percent: number, estimatedSeconds: number): string {
...
export function getAnalysisMessageHtml(data: {
...
export function getSuccessMessageHtml(data: { title: string; space: string; url: string; decisions: string[] }): string {
...
export function getErrorMessageHtml(data: { message: string; suggestion: string }): string {
...
export class ConfluenceChatViewProvider {
  private _panel: vscode.WebviewPanel | undefined;
  private _disposables: vscode.Disposable[] = [];
  private _messageHandler: ((msg: unknown) => void | Promise<void>) | undefined;

  constructor(private readonly _extensionUri: vscode.Uri) {}

  getOrCreatePanel(): vscode.WebviewPanel {
    if (this._panel) {
      return this._panel;
    }
    this._panel = vscode.window.createWebviewPanel(
      'confluenceChat',
      'Confluence - Intelligent Mode',
  ...
  setMessageHandler(handler: (msg: unknown) => void | Promise<void>): void {
  ...
  postMessage(msg: Record<string, unknown> & { type: string }): void {
...
export function asDisposable(provider: ConfluenceChatViewProvider): vscode.Disposable {
