...
class ConfluenceCredentials(BaseModel):
    ...
    url: str = Field(..., min_length=1, max_length=MAX_URL_LEN, description="Confluence base URL")
    email: str = Field(..., min_length=1, max_length=MAX_EMAIL_LEN, description="User email")
    api_token: str = Field(..., min_length=1, max_length=MAX_TOKEN_LEN, description="API token")
...
class AdvancedConfig(BaseModel):
    exportPath: str = Field(default="${workspaceFolder}/.confluence/examples", max_length=MAX_PATH_LEN)
...
class RisksConfig(BaseModel):
    ...
    fallbackTemplates: list[str] = Field(default_factory=lambda: ["mixed_project", "generic"], max_length=20)
    maxFileSize: str = Field(default="10MB", max_length=32)
...
def get_intelligent_defaults_model(url: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "learning": {"enabled": True, "retentionDays": 90, "minConfidence": 0.7, "autoImprove": True},
        "intelligence": {
