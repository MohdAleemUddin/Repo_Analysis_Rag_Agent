...
def _save_file_examples(examples: list[dict[str, Any]], base_dir: Optional[str] = None) -> None:
    path = _examples_file_path(base_dir)
    data = {"version": 1, "updated_at": datetime.now(timezone.utc).isoformat(), "examples": examples}
...
def store_example(
        ...
        try:
            ...
            if creation_id:
                db_execute(
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s)
                    """,
                    (str(uuid4()), "template", confidence_score or 0.0, creation_id, learned_at, confidence_score or 0.0),
                )
    ...
    record = {
        ...
        "intelligence_embedding": intelligence_embedding if intelligence_embedding and len(intelligence_embedding) == 1536 else None,
        "user_feedback": user_feedback if user_feedback is not None and 1 <= user_feedback <= 5 else None,
    }
...
def get_examples(
    ...
    if template_type or template_id:
        if template_id:
            examples = [e for e in examples if (e.get("template_ref") or {}).get("template_id") == template_id]
        if template_type:
            name = (template_type or "").lower()
            examples = [e for e in examples if name in ((e.get("template_ref") or {}).get("template_name") or "").lower()]
    return examples
...
def _save_file_feedback(items: list[dict[str, Any]], base_dir: Optional[str] = None) -> None:
    path = _feedback_file_path(base_dir)
    data = {"version": 1, "updated_at": datetime.now(timezone.utc).isoformat(), "feedback": items}
    with open(path, "w", encoding="utf-8") as f:
        json.dump(data, f, indent=2)
