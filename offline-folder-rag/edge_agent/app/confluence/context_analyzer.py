...
MODULE_PATTERN = re.compile(r"(?:from|import)\s+([a-zA-Z_][a-zA-Z0-9_.]*)\s+(?:import|$)")
            ...
            try:
                for name in os.listdir(abs_dir)[:50]:
                    ...
                    if full not in mentioned and full not in related:
                        if any(full.endswith(ext) for ext in (".py", ".ts", ".tsx", ".js", ".jsx", ".md", ".json")):
                            related.append(full)
            except OSError:
                continue
    except Exception:
...
def _detect_project_type(messages: list[dict[str, Any]], selected_text: str) -> str:
    ...
    for m in messages:
        if isinstance(m, dict):
            combined += " " + (m.get("content") or m.get("text") or str(m.get("message", "")))
        else:
            combined += " " + str(m)
    ...
    if "readme" in combined and not any(x in combined for x in ["def ", "import ", ".py"]):
...
def _detect_language(mentioned: list[str], selected_text: str) -> str:
    ...
    if (selected_text or "").strip():
        for ext, label in [("py", "Python"), ("ts", "TypeScript"), ("tsx", "TypeScript"), ("js", "JavaScript"), ("jsx", "JavaScript"), ("go", "Go"), ("rs", "Rust"), ("rb", "Ruby"), ("java", "Java")]:
            if f".{ext}" in (selected_text or "").lower() or "def " in (selected_text or "") or "import " in (selected_text or ""):
                if ext == "py":
                    return "Python"
                if ext in ("ts", "tsx"):
                    return "TypeScript"
...
def _should_suggest_readme(mentioned: list[str]) -> bool:
    ...
    has_readme = any(
        p.lower().endswith(("readme.md", "readme.txt")) or p.lower().replace("\\", "/").rstrip("/").endswith("/readme")
        for p in mentioned
    )
...
def analyze_chat_context(
    ...
    last_n = messages[-MAX_CHAT_MESSAGES:] if len(messages) > MAX_CHAT_MESSAGES else messages
    ...
    related = _resolve_related_files(mentioned_dedup[:20], workspace_path) if workspace_path else []
