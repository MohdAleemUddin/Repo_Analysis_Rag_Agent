    ...
    try:
        for entry in Path(root).rglob("*"):
            ...
            try:
                ...
                if is_dir:
                    if _should_exclude(full, name, True, root) or name.lower() in {d.lower() for d in _EXCLUDED_DIRS}:
                        continue
                    count += 1
                ...
                if ext in _PREFERRED_EXTENSIONS or base in _PREFERRED_NAMES or name in _PREFERRED_NAMES:
                    collected.append(full)
                elif ext in {".py", ".ts", ".tsx", ".js", ".jsx", ".java", ".go", ".md", ".json", ".yaml", ".yml"}:
                    collected.append(full)
