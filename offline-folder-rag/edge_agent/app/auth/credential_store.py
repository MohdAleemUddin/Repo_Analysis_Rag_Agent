...
def get_workspace_credentials(
    ...
    if service != "confluence":
        raise ValueError(f"Unknown service: {service}. Confluence credentials must be stored via this store.")
    prefix = ""
    if workspace_id:
        safe_id = workspace_id.replace(" ", "_").upper()
        prefix = f"CONFLUENCE_WS_{safe_id}_"
    base_url = os.environ.get(f"{prefix}BASE_URL") or os.environ.get("CONFLUENCE_BASE_URL")
    email = os.environ.get(f"{prefix}EMAIL") or os.environ.get("CONFLUENCE_EMAIL")
    api_token = os.environ.get(f"{prefix}API_TOKEN") or os.environ.get("CONFLUENCE_API_TOKEN")
